...
using Update = Cloudsume.Server.Models.DataProperty<System.Uri?>;

internal sealed class WebsiteManager : UniqueDataManager<Domain, Update>
{
    public WebsiteManager(IOptions<JsonOptions> json)
        : base(json)
    {
    }
    ...
    {
        dto.AddValidator(new UriAttribute()
        {
            Schemes = UriSchemes.HTTP | UriSchemes.HTTPS,
